...
Proving the recorded endpoint itself gone, which is what a caller needs before it retires durable records, is a separate and narrower read: a successful inventory of the exact recorded session that omits the exact recorded window.
That proof never accepts this `missing`, which is wider because it also answers a missing-server, refused-connection, or missing-session response - the server being unreachable rather than this window being gone - so an unreachable server, a failed inventory, and a malformed target all refuse there.
