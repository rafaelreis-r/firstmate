...
A subagent from omp's own task tool runs as a fully separate session that fires this same `session_start`/`session_shutdown` pair and never participates in that contract; the extension header owns the API signal it uses to tell a subagent apart from a genuine replacement, and how a wake is held back while one is still open.
