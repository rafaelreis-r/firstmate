...
For omp secondmate launches, `fm-spawn.sh` passes no `-e` at all: omp auto-discovers the home's tracked `.omp/extensions/` with no trust gate, and naming a discovered file with `-e` as well loads it twice; every omp launch instead carries a posture overlay through `--config`, which [`fm-spawn.sh --help`](../bin/fm-spawn.sh) owns.
A secondmate carries the tracked `.omp/fm-worker-overlay.yml` as is.
A crewmate or scout carries `state/<id>.omp-overlay.yml`, that template with its `retry:` block replaced per task: the usage-reserve policy is `fail-closed`, and omp's model fallback is either off, so a dead provider stops the worker and returns the decision to firstmate instead of hopping onto one of the captain's own `~/.omp/agent/config.yml` chains, or bound to the chain firstmate passes with `--fallback <provider>/<id>:<thinking>,...` from the approved dispatch tier.
The chain is recorded as `fallback=` in the task's meta beside `model_actual=`, the model the work actually runs on: `model=` at launch, rewritten by the per-task extension when omp's retry engine switches, with a `note: model fallback <from> -> <to>` status line for the same switch.
...
Codex `max` is valid when the profile selects a Luna model (`gpt-<version>-luna`, such as `gpt-5.6-luna` or `gpt-6-luna`), whose installed catalog entries support that reasoning level.
...
When the `clear` profile's harness is `omp`, its `profile:` line also carries `--fallback <chain>`, built in code from the same matched tier's other eligible omp candidates: ranked ones first in the same descending-`spendPriority` order the argmax used, then eligible-but-unranked ones in rule config order, each rendered `<model>:<effort>`, skipping the selected model itself, every non-`omp` candidate, a candidate missing `effort` or a `<provider>/<id>` model, and any repeated model; the chain is omitted entirely when no candidate remains, and each excluded eligible candidate's `candidate:` line names the reason it was left out of the chain.
...
FM_WATCH_SUBAGENT_CLEAR_TIMEOUT_MS=600000   # milliseconds the omp watch extension holds back a wake while a task-tool subagent session is open before sending anyway; bounds a subagent that never reaches session_shutdown
